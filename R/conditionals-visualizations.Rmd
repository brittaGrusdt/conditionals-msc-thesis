...
savePlot <- function(bias, lt, alpha, n_run, width, height, fn_main='', lawn_theta=0.05){
  ...
  target_dir <- get_target_dir("plots", n_run)
...
plot_pc_pa_facet_lt <- function(bias, alpha, n_run, lawn_theta=0.05){
  print("data for PL and LL needed!")
  # ---- facet for LL and PL ----- #
  ...
  p
  if(SAVE){savePlot(bias, "PL", alpha, n_run, 4, 2.5, "-pc-pa-facet-lt", lawn_theta)}

  return(p)
}

plot_pc_pa_facet_p <- function(bias, alpha, n_run, lawn_theta=0.05){
  print("data for PL and LL needed!")
...
plotHist <- function(proposition, bias, lt, n_run, title, from, to, alpha=5, bins=20){
...
plotTables <- function(bias, lt, n_run, alpha, lawn_theta=0.05){
...
plotCNs <- function(bias, lt, n_run, alpha, lawn_theta=0.05){
  data_samples <- read_data(bias, lt, n_run, alpha, lawn_theta)
...
plotCNs_LLPL <- function(bias, alpha, n_run, lawn_theta=0.05){
  df <- get_samples_LL_PL(bias, alpha, n_run, lawn_theta)
  ...
  p
  if(SAVE){savePlot(bias, "PL", alpha, n_run, 4, 6, "-facet-lt-cns", lawn_theta)}
  return(p)
}

plotTableDensityAllCNs <- function(bias, lt, n_run, alpha, lawn_theta=0.05){
  data_samples <- read_data(bias, lt, n_run, alpha, lawn_theta)
...
```{r Uncertainty_A_C_1}
p <-plot_pc_pa_facet_lt("none", ALPHA, N_RUN)
p
...
plotTableDensityAllCNs("none", "PL", N_RUN, ALPHA)
plotTableDensityAllCNs("none", "prior", N_RUN, ALPHA)
