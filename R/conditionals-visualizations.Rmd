...
get_pc_pa_mus <- function(df){
  ...
  mu_pc$variable <- c("pc","pc")
  mu_pa$variable <- c("pa","pa")
...
plot_pc_pa_facet_lt <- function(bias, alpha, n_run, lawn_theta=0.05, save=TRUE){
  # ---- facet for LL and PL ----- #
  df <- get_samples_LL_PL(bias, alpha, n_run, lawn_theta)
  ...
  if(save){
    ...
    ggsave(paste(target_dir, fn, "-pc-pa-facet-lt.png", sep=""), width = 4, height=2.5)
  }
  return(p)
}

plot_pc_pa_facet_p <- function(bias, alpha, n_run, lawn_theta, save=TRUE){
  ...
  if(save){
    fn <- get_filename(bias, "PL", alpha, n_run, lawn_theta)
    target_dir <- paste("../plots/seed-", SEEDS[n_run], "/", sep="")
    ggsave(paste(target_dir, fn, "-pc-pa-facet-p.png",sep=""), width = 4, height=2.5)
  }
  return(p)
}

plotHist <- function(proposition, bias, lt, n_run, title, from, to, alpha=5, bins=20, save=TRUE){
  samples <- read_data(bias, lt, n_run, alpha, lawn_theta)
  ...
  if(save){
    ...
    ggsave(paste(target_dir, fn, "-hist-p", proposition, ".png", sep=""), width=3, height=3)
  }
  return(p)
}

plotTables <- function(bias, lt, n_run, alpha, lawn_theta=0.05, save=TRUE){
  data_samples <- read_data(bias, lt, n_run, alpha, lawn_theta)
  t0 <- data_samples$pca 
  ...
  if(save){
    target_dir <- paste("../plots/seed-", SEEDS[n_run], "/", sep="")
    fn <- get_filename(bias, lt, alpha, n_run, lawn_theta)
    ggsave(paste(target_dir, fn, "-tables.png", sep=""), width=3, height=3)
  }
  return(p)
}

plotCNs <- function(bias, lt, n_run, alpha, lawn_theta=0.05, save=TRUE){
  data_samples <- read_data(bias, lt, n_run, alpha, lawn_theta)
  ...
  if(save){
    target_dir <- paste("../plots/seed-", SEEDS[n_run], "/", sep="")
    fn <- get_filename(bias, lt, alpha, n_run, lawn_theta)
    ggsave(paste(target_dir, fn, "-cns.png", sep=""), width=5, height=3.5)
  }
  return(p)
}

plotCNs_LLPL <- function(bias, alpha, n_run, lawn_theta=0.05, save=TRUE){
  df <- get_samples_LL_PL(bias, alpha, n_run, lawn_theta)
  ...
  if(save){
    target_dir <- paste("../plots/seed-", SEEDS[n_run], "/", sep="")
    fn <- get_filename(bias, "PL", alpha, n_run, lawn_theta)
    ggsave(paste(target_dir, fn, "-facet-lt-cns.png", sep=""), width=5, height=3.5)
  }
  return(p)
}

plotTableDensityAllCNs <- function(bias, lt, n_run, alpha, lawn_theta=0.05, save=TRUE){
  data_samples <- read_data(bias, lt, n_run, alpha, lawn_theta)
...
plotConditionalProbsDensities <- function(which_densities, bias, lt, n_run, alpha, lawn_theta=0.05){
  df <- read_data(bias, lt, n_run, alpha, lawn_theta)
...
visualizeSpeakerExpectation <- function(condition, bias, alpha, n_run, lawn_theta=0.05, save=TRUE){
  ...
  if(save){
    target_dir <- paste("../plots/seed-", SEEDS[n_run], "/", sep="")
    fn <- get_filename(bias, "PL", alpha, n_run, lawn_theta)
    ggsave(paste(target_dir, fn, "-SE-", condition, ".png", sep=""), width = 3.5, height=3)
  }
  return(p)
}
```
...
```{r Uncertainty_A_C_1}
plot_pc_pa_facet_lt("none", ALPHA, N_RUN)
```
...
plotTableDensityAllCNs("none", "PL", 1, 5)
```
